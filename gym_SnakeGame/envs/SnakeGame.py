...
class SnakeGameEnv(gym.Env):
    ...
    def __init__(self, size=7):
        self.size = size
        self.state = np.zeros((size, size, 4)) # 0: normal, 1: item, 2: Snake Head, 3: Snake Body
        self.snake = list()
        self.snake.append((0, 0))
        self.item_pos = (int(size / 2), int(size / 2))
        self.length = 1
    ...
    def step(self, action):
        ...
        if not done:
            ...
            if self.length == 64:
                ...
                self.state[head_x, head_y, 3] = 0
                self.state[head_x, head_y, 2] = 1
            else:
                self._update_tile()
    ...
    def _update_tile(self):
        ...
        if self.need_new_tile:
            ...
            while True:
                ...
                if self.state[item_x, item_y, 0] == 1:
                    self.item_pos = (item_x, item_y)
                    break

        item_x, item_y = self.item_pos
        self.state[item_x, item_y, 0] = 0
        self.state[item_x, item_y, 1] = 1
